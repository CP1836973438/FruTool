...
**中文文件名：** [使用手册.md](使用手册.md) · **纯文字详版：** [使用说明手册.md](使用说明手册.md) · **网页版：** [使用教程手册.html](使用教程手册.html) · **PDF：** [FRUTool-UserGuide.pdf](FRUTool-UserGuide.pdf)
...
| **FRU 字段刷写** | 读取当前板 FRU 并在字段框展示，再按字段改 Chassis / Board / Product | 改个别 FRU 字段、核对信息 |
...
- BMC 连上后会自动读取当前板 FRU，并在各**字段框里用灰字展示现有值**，便于核对
- 要改哪个字段，在对应框里输入新内容；灰字只是展示当前值，不会被当成写入内容
- **刷写所有非空字段**：只写你填了内容的格子；没输入的字段不会被覆盖
- **重置**：丢掉未提交的编辑
...
1. **第一步连接** 已就绪（BMC 在线）。
2. 侧栏进入 **FRU 字段刷写**。
3. 等各字段框出现当前板 FRU 值（灰字）。若没有，确认 BMC 在线后稍等，或点 **尝试读取 FRU**。
4. 在需要改的框里输入新值（不必填满；空字段不会被刷）。
5. 确认无误后点 **刷写所有非空字段**。
