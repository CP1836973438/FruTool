...
def validate_topo_write(path: str, user: str, pwd: str) -> Optional[DialogError]:
    ...
    if not os.path.isfile(topo_script):
        ...
        return ("脚本不存在", f"找不到 PcieEEpromTool.py:\n{topo_script}", "critical")
...
def run_topo_write(path: str, user: str, pwd: str, bmc_ip: str, log: LogCallback) -> dict[str, bool]:

    return {"ok": run_pcie_topology_write(path.strip(), user, pwd, bmc_ip, log)}
